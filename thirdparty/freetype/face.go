...
// GlyphEx Gets the glyph as an image with subpixel quality
func (f *Face) GlyphEx(ch rune) (*image.RGBA, *Metrics, error) {
	f.Lock()
	defer f.Unlock()

	loadFlags := C.FT_LOAD_RENDER | C.FT_LOAD_TARGET_LCD
	...
	metrics.AdvanceHeight = int(m.vertAdvance >> 6)
        ...
        sym_w := int(f.native.glyph.bitmap.width) / 3
	img := &image.RGBA{
		Pix:    pix,
		Stride: int(sym_w) * 4,
		Rect: image.Rect(
...
func rasterize(bitmap *C.FT_Bitmap) ([]byte, error) {
	...
	case C.FT_PIXEL_MODE_LCD:
		for i := 0; i < int(bitmap.rows); i++ {
			start := i * int(bitmap.pitch)
			stop := start + int(bitmap.width)

                        for j := start; j < stop; j = j + 3 {
                                pix = append(pix, data[j]) //R
                                pix = append(pix, data[j+1]) //G
                                pix = append(pix, data[j+2]) //B
                                pix = append(pix, 255) //A
			}
		}
